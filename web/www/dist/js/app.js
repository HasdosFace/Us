...
(BonziData.event_list_joke = [
	...
	(function () {
		//Im not doing the whole /bees thingy ever son of a bitch.
		const event_list_bees = [
			{ type: "text", text: "According to all known laws of aviation, there is no way a bee should be able to fly." },
			{ type: "text", text: "Its wings are too small to get its fat little body off the ground." },
			{ type: "text", text: "The bee, of course, flies anyway because bees don't care what humans think is impossible." },
		];
		try {
...
var socket = io(server_io, {
    query: {
		version: "1.01",
        channel: "bonziuniverse-itzcrazychace-edition",
    ...
    $.contextMenu({
        ...
        $.contextMenu({
            ...
            items: {
                ...
                custom: { name: "Custom", callback: function () { var url = prompt('Insert Supported Image URL for usage as the Background. Default is Windows XP BG.', `${window.location.origin}/img/desktop/__Themes/XP/wallpaper-xp.jpg`); if (url == "") { theme(); }; if (url) { theme(`#content{background-image:url("./img/desktop/logo.png"), url("${url}"); background-repeat: no-repeat, repeat; background-size: auto, cover;}'`); } } },
		    //BonziVERSE was in 2022
